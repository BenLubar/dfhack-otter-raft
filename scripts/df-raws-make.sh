...
DF() {
        version=${1//\./_}
        version=${version#0_}
        name=../archive/df_${version}_linux.tar.bz2
        if [[ -f "$name" ]]; then
                rm -rf -- *
                tar xf "$name" df_linux/raw df_linux/data/{announcement,dipscript,help,init,speech} df_linux/data/index
        else
                name=../archive/df_${version}.zip
                if [[ ! -f "$name" ]]; then
                        name=../archive/df_${version}_s.zip
                fi
                if [[ ! -f "$name" ]]; then
                        name=../archive/df_${version}_legacy.zip
                fi
                if [[ ! -f "$name" ]]; then
                        name=../archive/df_${version}_legacy_s.zip
                fi

                if [[ ! -f "$name" ]]; then
                        echo "(skipping $1 - no archive)" >&2
                        return 1
                fi
                ...
                mkdir df_linux
                cd df_linux
                unzip -q ../"$name" raw'/*' data/{announcement,dipscript,help,init,speech}'/*' data/index
                cd ..
        fi
